...
class NovaPlugin implements Plugin.PluginBase {
  ...
  private async bypassCloudflareImages(
    ...
    $content.find('img').each((i, img) => {
      const $img = $(img);
      const src =
        $img.attr('src') || $img.attr('data-src') || $img.attr('data-cfsrc');
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    let $chapterText: cheerio.Cheerio<Element>;
    ...
    const chapterContent = await this.bypassCloudflareImages($, $chapterText);
